...
#' tolerance <- 1e-6
#' seed <- 123
#' post_correction_results<-pruning_pleioh2g_wrapper(G,phenotype,munged_sumstats,ld_path,
#' wld_path, sample_prev, population_prev,n_block, hmp3,sample_rep,tolerance,seed)
...
pruning_pleioh2g_wrapper<-function(G,phenotype,munged_sumstats,ld_path, wld_path, sample_prev = NULL, population_prev = NULL,n_block = 200, hmp3,sample_rep,tolerance,seed){
  ...
  if(!all(Results_full_rg[lower.tri(Results_full_rg)]<rg_threshold)){
    ...
    cat("target disease =", current_D, ": pre-correction h2pleio/h2:", precorrresults$percentage_h2pleio_uncorr, "; s.e. ", precorrresults$percentage_h2pleio_uncorr_se,"\n")
    rg_threshold<-sqrt(0.5)
  ...
  tryCatch({
    if (rg_threshold > sqrt(0.1)) {
      ...
      cat("target disease =", current_D, ": ","post-correction h2pleio/h2 is ", postcorrresults$percentage_h2pleio_corr," ;s.e. ",postcorrresults$percentage_h2pleio_corr_se )
