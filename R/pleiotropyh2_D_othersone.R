...
#' @param otherCat one removing other auxiliary categort name - the same as category information in phenotype file
#' @param phenotype_path Path to the phenotype name and h2z score file - 1st col: 'traits'; 2nd col: 'h2_z'
#' @param allauxD_results_path path to the result using all auxiliary disease categories - the same target disease as G
...
#' package = "pleioh2g")
#' save_path <- system.file("extdata", "save_results_test/D_others",package = "pleioh2g")
#' sample_rep <- 10
#' tolerance <- 1e-6
#' seed <- 123
#' allauxD_results_path<-system.file("extdata/save_results_test/D", "318_h2pleio_dresults.csv",
#' package = "pleioh2g")
#' otherCat<-"digestive"
#' pleiotropyh2_D_othersone(G,phenotype_path,save_path,allauxD_results_path,otherCat,h2_vector_45D,h2_vector_mat_45D,Results_full_rg_45D,
#' Results_full_rg_array_45D,Rg_mat_z_45D,sample_rep,tolerance,seed)

pleiotropyh2_D_othersone<-function(G,phenotype_path,save_path,allauxD_results_path,otherCat,h2_vector,h2_vector_mat,Results_full_rg,Results_full_rg_array,Rg_mat_z,sample_rep,tolerance,seed){
  ...
  file_DG<-data.table::fread(allauxD_results_path)
  ...
  current_reC<-otherCat
  save_path_update<-paste0(save_path,'/de_',current_reC)
  dir.create(save_path_update)

  current_exT_exC<-setdiff(pruneddisease,pruneddisease_category$traits[which(pruneddisease_category$Category==current_reC)])

  phenotypename_update<-data.frame(
    traits = c(current_exT_exC),
    h2_z = pruneddisease_category$h2_z[match(c(current_exT_exC),pruneddisease_category$traits)]
  )
  phenotype_path_update<-paste0(save_path_update,'/',currentT,'_de_',current_reC,'_initialpheno.txt')
