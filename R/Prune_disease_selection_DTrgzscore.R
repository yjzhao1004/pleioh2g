...
#' Rg_prune<-Prune_disease_selection_DTrgzscore(Target_disease, trait_name,Results_full_rg_15D,Rg_mat_z_15D,rg_threshold)
#'
Prune_disease_selection_DTrgzscore<-function(Target_disease,trait_name,Rg_mat,Rg_mat_z,rg_threshold){
  ...
  if(length(result)==0){
    ...
    cat("remain", paste0(colnames(Rg_mat_leave),collapse = ","), "in the aux. diseases.. \n")
