...
#' n_block=3
#' results_jk<-Cal_rg_h2g_jk_alltraits(n_block, hmp3, phenotype, munged_sumstats, ld_path, wld_path, sample_prev, population_prev)
...
Cal_rg_h2g_jk_alltraits <- function(n_block = 200, hmp3, phenotype, munged_sumstats, ld_path, wld_path, sample_prev = NULL, population_prev = NULL) {
  ...
  for(block in c(1:n_block)){
    ...
    rg_res <- ldscr::ldsc_rg(
      munged_sumstats = setNames(GWAS_remainblocks, paste0('GWAS_', target_phenotypes, '_remainblock')),
      ld = ld_path,
      wld = wld_path,
      n_blocks = 200,
      chisq_max = NA,
