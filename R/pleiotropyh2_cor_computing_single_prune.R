...
pleiotropyh2_cor_computing_single_prune<-function(G,phenotype,h2_vector,h2_vector_mat,Results_full_rg,Results_full_rg_array, sample_rep = 1000, tolerance = 1e-6,seed = 123){
  ...
  for(sample_id in 1:sample_rep){
    #print(sample_id)
    sample_pleio_cor[sample_id,1]  <- generate_proposal_sample_changea_cor(Results_full_rg,Results_full_rg_array,G, 1,seed)
  ...
  while (max(abs(upper_bound_single-lower_bound_single)) > tolerance) {
    ...
    if (mean_est_a_single < true_pleio_cor_test) {
      ...
      for(sample_id in 1:sample_rep){
        sample_pleio_a_test[sample_id,1]  <- generate_proposal_sample_changea_cor(Results_full_rg,Results_full_rg_array,G, ratio_a,seed)
      }
