...
#' package = "pleioh2g")
#' save_path <- system.file("extdata", "save_results_test/D_T",package = "pleioh2g")
#' sample_rep <- 10
#' tolerance <- 1e-6
#' seed <- 123
#' allauxD_results_path<-system.file("extdata/save_results_test/D", "318_h2pleio_dresults.csv",
#' package = "pleioh2g")
#' pleiotropyh2_D_T(G,phenotype_path,save_path,allauxD_results_path,h2_vector_45D,h2_vector_mat_45D,Results_full_rg_45D,
#' Results_full_rg_array_45D,Rg_mat_z_45D,sample_rep,tolerance,seed)

pleiotropyh2_D_T<-function(G,phenotype_path,save_path,allauxD_results_path,h2_vector,h2_vector_mat,Results_full_rg,Results_full_rg_array,Rg_mat_z,sample_rep,tolerance,seed){
  ...
  phenotypename_update <- data.frame(
    traits = c(currentT, current_exT),
    h2_z = pruneddisease_category$h2_z[match(c(currentT, current_exT),pruneddisease_category$traits)]
  )
  phenotype_path_update <- file.path(save_path,paste0(currentT, '_T_pheno.txt'))

  data.table::fwrite(phenotypename_update, file = phenotype_path_update, quote = FALSE, sep = '\t')
  ...
  pruning_pleioh2g_corr_single_rgzscore_cutatall_more(target_num,phenotype_path_update,save_path,h2_vector_update,h2_vector_mat_update,Results_full_rg_update,Results_full_rg_array_update,Rg_mat_z_update,sample_rep,tolerance,seed)
